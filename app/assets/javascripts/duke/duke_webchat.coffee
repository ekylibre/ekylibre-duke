(($, E) ->
  ...
  # When Duke's data attribute is loaded, add account & pusher vals to vars
  ...
  $(document).behave "load", "duke[data-current-account]", ->
    ...
    # If there's alreay some msg, we show the btn-chat, otherwise we create a session and wait for the first msg
  ...
  # Creatig Duke session via Method in DukeWebchatController, storing assistant_id and session_id in sessionStorage
  create_session =  ->
