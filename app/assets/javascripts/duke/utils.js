(function (D, $) { 
  ...
  class DukeUtils {
    ...
    get session_inactivity() {
      return 86400000;
    }

    get base_urlReg() {
      return new RegExp('#base-url', 'g');
    }

    get redirectionReg() {
      return /redirect(keep)?=(.{10,})/;
    }

    get cancelationReg() {
      return new RegExp('annul', 'i');
    }
    ...
    get templates() {
      return { // Json object representing HTML partials
        ...
        checked_mult_option: (val, label)=> {return '<label data-value= \'' + this.escapeHtml(val) + '\'class="control control--checkbox">' + this.escapeHtml(label) + '<input type="checkbox"/> <div class="control__indicator duke-checked"></div> </label>'},
