(function (E, $) {
  ...
  class DukeWebchat{ 
    ...
    init() {
      $.ajax('/duke_api_details', {
        type: 'get',
        dataType: 'json',
        success: ((data) => {this.pusherHandler = new PusherHandler(data.pusher_key, this);
                             this.stt = new STTHandler(data.azure_key, data.azure_region, this);
  ...
  class DukeMessage{
    ...
    redirect() {
      ...
      if (redirection) {
        ...
        duke.webchat.pusherHandler.reset();
  ...
  E.onDomReady(function () {
    if (duke.webchat){
      duke.webchat.pusherHandler.reset();
      duke.webchat.display();
    } else {
      duke.webchat = new DukeWebchat($("duke").data('current-account'), $("duke").data('current-tenant'));
    };
