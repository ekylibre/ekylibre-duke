      ...
      class SaveHarvestReception < Duke::Skill::DukeHarvestReception
        ...
        private
          ...
          def save_harvest_reception
            ...
            targets_attributes.each{|target| WineIncomingHarvestPlant.create(target)}
            storages_attributes.each{|storage| WineIncomingHarvestStorage.create(storage)}
            press_attributes.each{|press| WineIncomingHarvestPress.create(press)}
            ih.id
          end

          #  @returns [Hash] Analysis_attributes with every analysis items
          def create_analysis_attributes
            attributes = { '0'=>{ '_destroy'=>'false', 'indicator_name'=>'estimated_harvest_alcoholic_volumetric_concentration',
  'measure_value_value'=> @parameters['tav'], 'measure_value_unit'=>'volume_percent' } }
            unless @parameters['ph'].nil?
              attributes[1] =
  { '_destroy'=>'false', 'indicator_name'=>'potential_hydrogen', 'decimal_value'=> @parameters['ph'] }
            end
            unless @parameters['temperature'].nil?
              attributes[2] =
  { '_destroy'=>'false', 'indicator_name'=>'temperature', 'measure_value_value'=> @parameters['temperature'],
'measure_value_unit'=>'celsius' }
            end
            unless @parameters['assimilated_nitrogen'].nil?
              attributes[3] =
  { '_destroy'=>'false', 'indicator_name'=>'assimilated_nitrogen_concentration',
'measure_value_value'=> @parameters['assimilated_nitrogen'], 'measure_value_unit'=>'milligram_per_liter' }
            end
            unless @parameters['amino_nitrogen'].nil?
              attributes[4] =
  { '_destroy'=>'false', 'indicator_name'=>'amino_nitrogen_concentration', 'measure_value_value'=> @parameters['amino_nitrogen'],
'measure_value_unit'=>'milligram_per_liter' }
            end
            unless @parameters['ammoniacal_nitrogen'].nil?
              attributes[5] =
  { '_destroy'=>'false', 'indicator_name'=>'ammoniacal_nitrogen_concentration', 'measure_value_value'=> @parameters['ammoniacal_nitrogen'],
'measure_value_unit'=>'milligram_per_liter' }
            end
            unless @parameters['h2so4'].nil?
              attributes[6] =
  { '_destroy'=>'false', 'indicator_name'=>'total_acid_concentration', 'measure_value_value'=>@parameters['h2so4'],
'measure_value_unit'=>'gram_per_liter' }
            end
            unless @parameters['malic'].nil?
              attributes[7] =
  { '_destroy'=>'false', 'indicator_name'=>'malic_acid_concentration', 'measure_value_value'=>@parameters['malic'],
'measure_value_unit'=>'gram_per_liter' }
            end
            unless @parameters['sanitarystate'].nil?
              attributes[8] =
  { '_destroy'=>'false', 'indicator_name'=>'sanitary_vine_harvesting_state',
'string_value'=> @parameters['sanitarystate'] }
            end
            unless @parameters['pressing_tavp'].nil?
              attributes[9] =
  { 'measure_value_unit' =>'volume_percent', 'indicator_name' => 'estimated_pressed_harvest_alcoholic_volumetric_concentration',
          ...
          def ih_attr(dic)
            unless @parameters['complementary'].blank? # add complementary if exists
              if @parameters['complementary'].key?('ComplementaryDecantation')
                dic[:sedimentation_duration] =
  @parameters['complementary']['ComplementaryDecantation'].delete('^0-9')
              end
              if @parameters['complementary'].key?('ComplementaryTrailer')
                dic[:vehicle_trailer] =
  @parameters['complementary']['ComplementaryTrailer']
              end
              if @parameters['complementary'].key?('ComplementaryTime')
                dic[:harvest_transportation_duration] =
  @parameters['complementary']['ComplementaryTime'].delete('^0-9')
              end
              if @parameters['complementary'].key?('ComplementaryDock')
                dic[:harvest_dock] =
@parameters['complementary']['ComplementaryDock']
              end
              if @parameters['complementary'].key?('ComplementaryNature')
                dic[:harvest_nature] =
  @parameters['complementary']['ComplementaryNature']
              end
          ...
          def unit_to_hectoliter(value, unit)
            if unit == 'hl'
              return format('%.3f', value.to_f)
            elsif unit == 'kg'
              return format('%.3f', value.to_f/130)
            else
              return format('%.3f', value.to_f/0.130)
            end
          end
          ...
          def storages_attributes
            ...
            return @destination.map{|cuve|
   { storage_id: cuve.key, quantity_value: cuve['quantity'], quantity_unit: 'hectoliter', wine_incoming_harvest_id: @id }}
          end
   ...
   { plant_id: crop[:id], harvest_percentage_received: cg[:area].to_s, wine_incoming_harvest_id: @id }}}.flatten
          ...
          def press_attributes
            ...
            return @press.map{|press|
   { press_id: press.key, quantity_value: press['quantity'], quantity_unit: 'hectoliter', wine_incoming_harvest_id: @id,
pressing_schedule: press[:pressing_schedule], pressing_started_at: press[:pressing_started_at] }}
          end
          ...
          def add_pressing_program
            @press.each do |press|
              ...
              press[:pressing_started_at] =
  (@parameters['pressing']['hour'].to_time.strftime('%H:%M') if @parameters.dig(:pressing, :hour).present?)||''
            end
          end
